...
namespace Zeta.Extreme.Core.Tests.PrimaryTests
{
	...
	public class ZC248AltObjToSqlTest
	{
		...
		public void QueryWithAltObjWillBeParsedWell() {
			...
			Assert.AreEqual(@"
SELECT id,col,row,obj,year,1, decimalvalue , 1 ,'1,2,3'   FROM cell  WHERE period=1 and year=2012 and  col in ( 1) and obj=1 and row in (1) and altobj in (1,2,3)  ", script);
		}
