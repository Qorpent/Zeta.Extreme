...
namespace Zeta.Extreme
{
	...
	public class ZetaVirtualSumHelper
	{
		...
		public bool EvalIsSum(IZetaRow zetaRow) {
			if(zetaRow.IsMarkSeted("0SA")) return true;
			if(zetaRow.IsFormula && zetaRow.FormulaEvaluator=="boo" && IsSumableFormula(zetaRow.Formula)) return true;
			return false;
		}
		...
		private IEnumerable<ZexQueryDelta> GetFormulaSumDelta(IZetaRow row) {
			...
			foreach ( Match match in matches) {
				...
				if(!ps) p = -p;
				if(!s) delta.Multiplicator = -1;
				if(!string.IsNullOrWhiteSpace(r)) {
					var _r = RowCache.get(r);
					if(null!=_r) delta.Row = _r;
					else delta.RowCode = r;
				}
				if (!string.IsNullOrWhiteSpace(c))
				{
					var _c = ColumnCache.get(c);
					if (null != _c) delta.Column = _c;
					delta.ColumCode = c;
				}
				if(0!=y) {
					delta.Year = y ;
		...
		private IEnumerable<ZexQueryDelta> GetNativeSumDelta(IZetaRow row) {
			foreach (var c in row.Children) {
				if(c.IsMarkSeted("0CAPTION"))continue;
				if(c.IsMarkSeted("0NOSUM"))continue;
				if(IsSum(c)) {
