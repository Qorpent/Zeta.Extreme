...
!function($) {
    ...
    $(window.zefs).on("session_load", function() {
        session.attr("uid", window.zefs.myform.sessionId);
        debuginfo.attr("uid", window.zefs.myform.sessionId);
        lock.attr("uid", window.zefs.myform.sessionId);
        canlock.attr("uid", window.zefs.myform.sessionId);
    });
...
!function($) {
    ...
    b.click(function(e) {
        window.zefs.myform.save(window.zefs.myform.getChanges());
    });
    $(window.zefs).on("formstatus_load", function() {
        if (!zefs.myform.lock) {
            b.attr("disabled", "disabled");
        }
    });
    $(window.zefs).on("savestage_started", function() {
        b.attr("disabled", "disabled");
    });
...
!function($) {
    ...
    var m = $('<div class="btn-group"/>').append(
        $('<button class="btn btn-small dropdown-toggle" title="Информация" data-toggle="dropdown"/>').html('<i class="icon-book"></i><span class="caret"></span>'),
        $('<ul class="dropdown-menu"/>').append(
...
!function($) {
    var zefsform = new root.Widget("zefsform", root.Console.layout.position.layoutBodyMain, null, { authonly: true, ready: function() {
        window.zefs.init(jQuery);
        window.zefs.myform.run();
    } });
...
!function($) {
    ...
    var list = $('<div class="btn-group"/>').append(
        $('<button class="btn btn-small dropdown-toggle" title="Период" data-toggle="dropdown"/>').html('<i class="icon-calendar"></i><span class="caret"/>'),
        $('<ul class="dropdown-menu"/>').append(
