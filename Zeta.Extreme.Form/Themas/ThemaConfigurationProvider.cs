...
namespace Comdiv.Zeta.Web.Themas{
    public class ThemaConfigurationProvider : IThemaConfigurationProvider{
        ...
        private XElement CompileXml(){
            ...
            globals["“≈ ”Ÿ»…√Œƒ"] = y.ToString();
            globals["CURRENTYEAR"] = globals["“≈ ”Ÿ»…√Œƒ"];
            globals["—œ»—Œ √ŒƒŒ¬"] = y.range(y - 4).concat("|") + "|--|" + 1990.range(y + 3).concat("|");
            globals["YEARLIST"] = globals["—œ»—Œ √ŒƒŒ¬"];
            globals["“≈ ”Ÿ»…œ≈–»Œƒ"] = Periods.ForMonth(DateTime.Today.Month).ToString();
            globals["CURRENTPERIOD"] = globals["“≈ ”Ÿ»…œ≈–»Œƒ"];
            ...
            foreach (var element in inputs) {
                ...
                if (input.Template == "empty.in"){
                    ...
                    if (null == templateFile){
                        input.IsError = true;
                        input.Warrnings.Add("ÓÚÒÛÚÒÚ‚ÛÂÚ ¯‡·ÎÓÌ Ò ÍÓ‰ÓÏ " + input.Template);
                    }
                    else{
                        input.TemplateXml =
        ...
        private void readOutputs(ThemaConfiguration configuration){
            ...
            foreach (var element in outs){
                ...
                if (output.Template == "empty.out"){
                    ...
                    if (null == templateFile){
                        output.IsError = true;
                        output.Warrnings.Add("ÓÚÒÛÚÒÚ‚ÛÂÚ ¯‡·ÎÓÌ Ò ÍÓ‰ÓÏ " + output.Template);
                    }
                    else{
                        output.TemplateXml =
                            XElement.Parse(PathResolver.ReadXml(templateFile, null,
            ...
            foreach (var cfg in configurations){
                ...
                foreach (var import in imports){
                    ...
                    if(!configurations.ContainsKey(code)) {
                        throw new Exception(string.Format("ÌÂ‚ÓÁÏÓÊÌÓ ÔÓËÁ‚ÂÒÚË ËÏÔÓÚ ÚÂÏ˚ {0} ‚ ÚÂÏÛ {1} ({2})",code, cfg.Value.Code, cfg.Value.Evidence));
                    }
    	...
    	protected virtual XElement readBaseXml(){
            ...
            foreach (var thema in allthemas){
                ...
                defines["“≈ ”Ÿ»…√Œƒ"] = y.ToString();
                defines["CURRENTYEAR"] = y.ToString();
                defines["—œ»—Œ √ŒƒŒ¬"] = 1990.range(y + 3).concat("|");
                defines["YEARLIST"] = 1990.range(y + 3).concat("|");
                defines["“≈ ”Ÿ»…œ≈–»Œƒ"] = Periods.ForMonth(DateTime.Today.Month).ToString();
