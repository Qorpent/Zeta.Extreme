...
namespace Comdiv.Zeta.Web.InputTemplates {
    public class InputTemplateToFormDefinitionConverter {
        public Form PrepareForm(IInputTemplate template, IZetaMainObject obj) {
            var result = new Form();
            result.ObjectId = obj.Id;
        	result.Thema = template.Thema;
            result.Tag = obj;
            var columns = template.GetAllColumns().Where(v => v.GetIsVisible(obj)
				&&((template.GetColGroup().noContent() && v.Group.noContent()) ||
						v.Group == template.GetColGroup()
					)
				).ToList();
            var rows = template.Rows;
            foreach (var column in columns.Where(x=>x.Visible)) {
                if (null == result.Columns.FirstOrDefault(x => x.Code == column.EffectiveCode && x.Year == column.Year && x.Period == column.Period)) {
            ...
            foreach (var row in rows) {
                if (row.Target != null) {
                    addRow(result, row.Target,null,false);
                }else {
                    var r = result.AddRow();
                    r.Code = row.Code;
                    r.Tag = row;
        ...
        private void addRow(Form result, IZetaRow row, Row parent, bool selfc) {
            ...
            r.IsAggregate = row.IsMarkSeted("0SA") && !row.IsFormula;
